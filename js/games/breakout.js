...
function resetGame(){
paddle={x:W/2,y:H-30};score=0;lives=5;level=1;gameTime=0;particles=[];powerups=[];BALL_SPEED=280;
...
if(alive===0){level++;BALL_SPEED=Math.min(450,280+level*15);buildBricks();resetBall();}
// particles
...
if(b.hits>1){ctx.fillStyle='rgba(255,255,255,0.4)';ctx.font='bold 10px "Courier New",monospace';ctx.textAlign='center';ctx.fillText('■',b.x+b.w/2,b.y+b.h/2+4);}}
// paddle
...
// ball trail
ctx.fillStyle='rgba(255,255,255,0.15)';ctx.beginPath();ctx.arc(ball.x-ball.vx*0.02,ball.y-ball.vy*0.02,BALL_R*0.7,0,Math.PI*2);ctx.fill();
ctx.fillStyle='rgba(255,255,255,0.08)';ctx.beginPath();ctx.arc(ball.x-ball.vx*0.04,ball.y-ball.vy*0.04,BALL_R*0.5,0,Math.PI*2);ctx.fill();
// powerups
...
ctx.globalAlpha=1;
// lives
for(var i=0;i<lives;i++){ctx.fillStyle='#ff66aa';ctx.beginPath();ctx.arc(18+i*22,H-14,6,0,Math.PI*2);ctx.fill();}
// level
