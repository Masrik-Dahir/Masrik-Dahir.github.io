...
(function () {
    ...
    function navButton(iconKey, onclick, vShow, extraClass, href, imgStyle) {
        ...
        var inner = '<img src="' + ICONS[iconKey] + '" ' + imgStyle + '>';
    ...
    function buildDesktopControls() {
        return '<div class="resizing-1026-more"><table width="100%"><tr><td><div style="padding: 10px">' +
            '<button class="glow-on-hover4 Blazing" style="width: fit-content;margin: 5px"><a href="http://www.masrikdahir.com"><img src="' + ICONS.home + '" ' + btnStyle + '></a></button>' +
            '<button v-show="isSlideVisible" class="glow-on-hover4 Blazing" style="width: fit-content;margin: 5px" onclick="prevSlide()"><img src="' + ICONS.left + '" ' + btnStyle + '></button>' +
            '<button v-show="isSlideVisible" class="glow-on-hover4 Blazing" style="width: fit-content;margin: 5px" @click="showGallery" onclick="showGalById()"><img src="' + ICONS.gallery + '" ' + btnStyle.replace(';"', ';padding-top: 3px;padding-bottom: 3px;"') + '></button>' +
            '<button v-show="!isSlideVisible" class="glow-on-hover4 Blazing" style="width: fit-content;margin: 5px" @click="showSlide" onclick="showSlideById()"><img src="' + ICONS.slideshow + '" ' + btnStyle + '></button>' +
            '<button v-show="isSlideVisible" class="glow-on-hover4" style="width: fit-content;margin: 5px" onclick="pauseSlideShow()"><img src="' + ICONS.pause + '" ' + btnStyle + '></button>' +
            '<button v-show="isSlideVisible" class="glow-on-hover4" style="width: fit-content;margin: 5px" onclick="resumeSlideShow()"><img src="' + ICONS.resume + '" ' + btnStyle + '></button>' +
            '<button v-show="isSlideVisible" class="glow-on-hover4 Blazing" style="width: fit-content;margin: 5px" onclick="nextSlide()"><img src="' + ICONS.right + '" ' + btnStyle + '></button>' +
            '<button class="glow-on-hover4 Blazing" style="width: fit-content;margin: 5px"><a href="http://www.masrikdahir.com/map" style="width: 40px"><img src="' + ICONS.map + '" ' + btnStyle + '></a></button>' +
            '</div></td></tr></table></div>';
    }
    ...
    function buildMobileControls() {
        ...
        return '<div class="resizing-1026-less"><table width="100%"><tr><td><div style="padding: 2px">' +
            '<button class="glow-on-hover4 Blazing" style="width: fit-content; padding: 2px; margin: 5px"><a href="http://www.masrikdahir.com"><img src="' + ICONS.home + '" ' + s + '></a></button>' +
            '<button v-show="isSlideVisible" class="glow-on-hover4 Blazing" style="width: fit-content; padding: 2px; margin: 5px" onclick="prevSlide()"><img src="' + ICONS.left + '" ' + s + '></button>' +
            '<button v-show="isSlideVisible" class="glow-on-hover4 Blazing" style="width: fit-content; padding: 2px; margin: 5px" @click="showGallery" onclick="showGalById()"><img src="' + ICONS.gallery + '" ' + s + '></button>' +
            '<button v-show="!isSlideVisible" class="glow-on-hover4 Blazing" style="width: fit-content; padding: 2px; margin: 5px" @click="showSlide" onclick="showSlideById()"><img src="' + ICONS.slideshow + '" ' + s + '></button>' +
            '<button v-show="isSlideVisible" class="glow-on-hover4" style="width: fit-content; padding: 2px; margin: 5px" onclick="pauseSlideShow()"><img src="' + ICONS.pause + '" ' + s + '></button>' +
            '<button v-show="isSlideVisible" class="glow-on-hover4" style="width: fit-content; padding: 2px; margin: 5px" onclick="resumeSlideShow()"><img src="' + ICONS.resume + '" ' + s + '></button>' +
            '<button v-show="isSlideVisible" class="glow-on-hover4 Blazing" style="width: fit-content; padding: 2px; margin: 5px" onclick="nextSlide()"><img src="' + ICONS.right + '" ' + s + '></button>' +
            '<button class="glow-on-hover4 Blazing" style="width: fit-content; padding: 2px; margin: 5px"><a href="http://www.masrikdahir.com/map"><img src="' + ICONS.map + '" ' + s + '></a></button>' +
            '</div></td></tr></table></div>';
    }
    ...
    function buildGalleryTemplate() {
        return '<table width="100%" style="margin-top: 10px"><tr><td width="100%">' +
            ...
            // Title
            '<div class="tag w3-margin-bottom" style="margin-top:70px"><div class="it">{{ name }}</div></div>' +

            // Slideshow view
            '<div id="showSlide" v-show="isSlideVisible">' +
            '<div v-for="item in resources" :key="item.title" class="content-slide">' +
            '<img :src="item.url" style="width:100%; height: 100%; object-fit: contain; padding: 0"/>' +
            '</div></div>' +
            ...
            '<div v-if="!item.isActive" class="inactive-image image-container" @click="selectImage(index)"><img :src="item.url" /></div>' +
            '</div></div></div>' +
